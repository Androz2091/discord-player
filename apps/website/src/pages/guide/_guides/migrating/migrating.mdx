...
> _You can find the full list of available `nodeOptions` over [here](http://localhost:3000/docs/discord-player?type=type&target=GuildNodeInit)_
...
Player [events](/docs/discord-player?type=type&target=GuildQueueEvents) are now emitted from the `player.events` object. (ex. `player.events.on(event.name, (...args) => event.execute(...args));`)
...
There are several new filter options, with some shown below. A full list can be found [here](/docs/discord-player?type=type&target=QueueFilters).
