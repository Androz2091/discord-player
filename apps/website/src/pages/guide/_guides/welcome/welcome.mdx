...
Let's create a main player instance in `index.js`. This instance handles and keeps track of all the queues and its components. Discord Player is mostly events based. It emits different events based on the context and actions. Let's also add a basic event listener in `events.js` to notify the user when a track starts to play.
