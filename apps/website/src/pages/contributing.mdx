<div className="min-h-[75vh] flex items-center justify-center flex-col py-4">

<div className="text-start w-1/2">

<h1 className="text-xl text-center font-bold mb-4">Contributing Guide</h1>
...
First of all, lets understand what this project is about. Discord Player is a high level framework built on top of <a className="text-blue-500" href="https://github.com/discord-player/discord-voip">discord-voip</a>, a fork of <a href="https://github.com/discordjs/discord.js" className="text-blue-500">@discordjs/voice</a>. `discord-voip` is a low level library to interact with <a className="text-blue-500" href="https://discord.com">Discord</a>'s VoIP. In other words, lets imagine discord-voip library as a brick and discord-player as a house built with that brick.
...
<h1 className="text-lg font-bold">Discord Player</h1>
...
Discord Player has been utilizing <a className="text-blue-500" href="https://ffmpeg.org">FFmpeg</a> as a way to transcode incoming source into consumable format, which is mostly a signed 16-bit little endian PCM audio. But sometimes in order to save resources, discord-player tries to decode the incoming source without ffmpeg when possible. The intermmediate steps in between this may look complex but it is fairly simple.
...
<h1 className="text-lg font-bold">Project Structure</h1>

The discord-player project is structured into several packages:

-   **`1. discord-player`**: The main source code for discord-player.
-   **`2. @discord-player/utils`**: Basic utilities required by discord-player.
-   **`3. @discord-player/equalizer`**: Digital signal processing utilities.
-   **`4. @discord-player/extractor`**: A collection of extractors for bootstrapping music bots.
-   **`5. @discord-player/ffmpeg`**: Streamable API for managing FFmpeg processes.
-   **`6. @discord-player/opus`**: Streamable API over libopus bindings.

<h1 className="text-lg font-bold">Contributing</h1>
...
**8. Commits:**
Although it is not necessary, it is recommended to use <a href="https://www.conventionalcommits.org/en/v1.0.0/" className="text-blue-500">conventional commits message</a> to briefly describe what the update does.
