...
<Callout>
  This feature is not currently supported with `player.play()`. If you depend upon `player.play()` for queue creation,
  you need to manually create the queue. You can then use `queue.play()` instead to achieve `player.play()`
  functionality.
</Callout>
