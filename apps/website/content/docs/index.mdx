...
<Callout type="warn">
  We do not recommend installing `ffmpeg` via npm because binaries pulled from npm is known to be unstable. It is
...
<Callout type="info">Use `FFMPEG_PATH` environment variable to load ffmpeg from custom path.</Callout>
