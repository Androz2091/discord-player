...
<Callout>
  We do not recommend using static binaries like `ffmpeg-static` as they are not always up-to-date or stable and may
  cause issues.
</Callout>
...
client.once('ready', () => {
  ...
  player.events.on('debug', (queue, message) => console.log(`[DEBUG ${queue.guild.id}] ${message}`)); // [!code highlight]
