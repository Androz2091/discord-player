...
The play command allows users to play music in a voice channel. The play command must cover the following functionalities:

1. Abort if the user is not in a voice channel.
2. Abort if the bot does not have permission to join the voice channel.
3. Abort if the bot does not have permission to speak in the voice channel.
4. Abort if the bot is already playing in a voice channel different from the user's voice channel.
5. Join the user's voice channel.
6. Play the requested song if the queue is empty, otherwise add the song to the queue.
7. Also handle playlists and other audio sources.
