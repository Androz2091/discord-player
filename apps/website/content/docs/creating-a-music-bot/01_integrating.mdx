...
<Callout type="warn">
  We don't recommend assigning `player` to `client` using `client.player = player` here. Discord Player provides
  [hooks](/docs/hooks/using_hooks) (aka helper functions) to access discord-player objects at ease.
</Callout>
