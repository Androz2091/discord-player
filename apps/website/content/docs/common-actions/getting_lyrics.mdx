...
Discord Player provides two different methods for getting lyrics of a song. The lyrics client is built into discord-player itself and provides both plaintext and synced lyrics. It is powered by [lrclib](https://liclib.net).
...
<Callout type="info">
  Discord Player relies on queue's current time to detect the current line in the lyrics. It does not validate if the
  current track is the same as the track for which lyrics were fetched.
</Callout>
