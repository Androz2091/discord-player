...
class QueryResolver {
    ...
    static resolve(query: string): QueryType {
        ...
        if (appleMusicTrack.test(query)) return QueryType.APPLE_MUSIC_TRACK;
        if (appleMusicPlaylist.test(query)) return QueryType.APPLE_MUSIC_PLAYLIST;
        if (appleMusicAlbum.test(query)) return QueryType.APPLE_MUSIC_ALBUM;
