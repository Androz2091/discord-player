...
export class TremoloCommand extends Command {
	...
	public override async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
		if (interaction.member instanceof GuildMember) {
			...
			if (ff.includes('Tremolo')) {
				ff = ff.filter((r) => r !== 'Tremolo');
			} else {
				ff.push('Tremolo');
			}
			...
			return interaction.followUp({
				content: `${this.container.client.dev.success} | **Tremolo** filter ${ff.includes('Tremolo') ? 'enabled' : 'disabled'}!`
			});
		}
	}
}
